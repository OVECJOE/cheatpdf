...
// Types
type EducationLevel = "High School" | "Undergraduate" | "Graduate" | "PhD" | "Professional" | "Other";
type SubjectCategory = "STEM" | "Liberal Arts" | "Social Sciences" | "Professional" | "Applied";
type ExamCategory = "standardized" | "academic" | "professional" | "international";
type StudyGoal = "improve-grades" | "exam-prep" | "understand-concepts" | "time-management" | "retention" | "problem-solving" | "research-skills" | "career-prep";
...
const EDUCATION_CONFIGS: Record<EducationLevel, EducationConfig> = {
  "High School": {
    level: 1,
    subjects: ["Mathematics", "Science", "History", "Literature", "Language Arts", "Chemistry", "Physics", "Biology"],
    exams: ["SAT/ACT", "AP Exams", "Final Exams", "State Tests"],
    goals: ["improve-grades", "exam-prep", "understand-concepts", "time-management"],
    description: "Building foundational knowledge"
  },
  "Undergraduate": {
    level: 2,
    subjects: ["Mathematics", "Computer Science", "Engineering", "Business", "Economics", "Psychology", "Chemistry", "Physics", "Biology"],
    exams: ["Final Exams", "Midterms", "GRE/GMAT", "Professional Certification"],
    goals: ["improve-grades", "understand-concepts", "exam-prep", "problem-solving", "career-prep"],
    description: "Developing specialized expertise"
  },
  "Graduate": {
    level: 3,
    subjects: ["Computer Science", "Engineering", "Medicine", "Law", "Business", "Economics", "Psychology", "Research Methods"],
    exams: ["Comprehensive Exams", "Thesis Defense", "Professional Certification", "Medical Boards", "Bar Exam"],
    goals: ["research-skills", "understand-concepts", "exam-prep", "career-prep", "problem-solving"],
    description: "Advanced study and research"
  },
  "PhD": {
    level: 4,
    subjects: ["Research Methods", "Advanced Theory", "Dissertation Topic", "Teaching Methods"],
    exams: ["Qualifying Exams", "Dissertation Defense", "Comprehensive Exams"],
    goals: ["research-skills", "understand-concepts", "career-prep", "time-management"],
    description: "Original research and expertise"
  },
  "Professional": {
    level: 5,
    subjects: ["Industry Standards", "Certification Requirements", "Continuing Education"],
    exams: ["Professional Certification", "Licensing Exams", "Industry Certifications"],
  ...
  "Other": {
    ...
    goals: ["understand-concepts", "retention", "time-management", "improve-grades"],
    description: "Flexible learning path"
  }
};

const SUBJECT_CONFIGS: Record<string, SubjectConfig> = {
  "Mathematics": { category: "STEM", relatedExams: ["SAT/ACT", "GRE/GMAT", "AP Exams"], suggestedGoals: ["problem-solving", "understand-concepts"] },
  "Computer Science": { category: "STEM", relatedExams: ["Technical Interviews", "Certification Exams"], suggestedGoals: ["problem-solving", "career-prep"] },
  "Engineering": { category: "STEM", relatedExams: ["PE Exam", "FE Exam"], suggestedGoals: ["problem-solving", "career-prep"] },
  "Medicine": { category: "Professional", relatedExams: ["Medical Boards", "MCAT"], suggestedGoals: ["exam-prep", "retention"] },
  "Law": { category: "Professional", relatedExams: ["Bar Exam", "LSAT"], suggestedGoals: ["exam-prep", "understand-concepts"] },
  "Business": { category: "Professional", relatedExams: ["CPA", "MBA Exams"], suggestedGoals: ["career-prep", "understand-concepts"] },
  "History": { category: "Liberal Arts", relatedExams: ["AP History", "Comprehensive Exams"], suggestedGoals: ["retention", "understand-concepts"] },
  "Literature": { category: "Liberal Arts", relatedExams: ["AP Literature", "Comprehensive Exams"], suggestedGoals: ["understand-concepts", "retention"] },
  "Psychology": { category: "Social Sciences", relatedExams: ["Psychology Boards", "Research Defense"], suggestedGoals: ["research-skills", "understand-concepts"] },
  "Economics": { category: "Social Sciences", relatedExams: ["Economics Comprehensive", "Professional Certification"], suggestedGoals: ["problem-solving", "understand-concepts"] }
};

const EXAM_CONFIGS: Record<string, ExamConfig> = {
  "SAT/ACT": { category: "standardized", difficulty: 3, timeframe: "medium", subjects: ["Mathematics", "Language Arts", "Science"] },
  "GRE/GMAT": { category: "standardized", difficulty: 4, timeframe: "long", subjects: ["Mathematics", "Language Arts", "Logic"] },
  "AP Exams": { category: "academic", difficulty: 3, timeframe: "medium", subjects: ["Subject-Specific"] },
  "Medical Boards": { category: "professional", difficulty: 5, timeframe: "long", subjects: ["Medicine", "Clinical Skills"] },
  "Bar Exam": { category: "professional", difficulty: 5, timeframe: "long", subjects: ["Law", "Legal Practice"] },
  "Final Exams": { category: "academic", difficulty: 2, timeframe: "short", subjects: ["Course-Specific"] },
  "Midterms": { category: "academic", difficulty: 2, timeframe: "short", subjects: ["Course-Specific"] }
};

const GOAL_CONFIGS: Record<StudyGoal, { label: string; priority: number; description: string }> = {
  "improve-grades": { label: "Improve grades", priority: 1, description: "Boost your academic performance" },
  "exam-prep": { label: "Exam preparation", priority: 2, description: "Ace your upcoming tests" },
  "understand-concepts": { label: "Better understanding", priority: 3, description: "Deepen your knowledge" },
  "time-management": { label: "Time management", priority: 4, description: "Study more efficiently" },
  "retention": { label: "Information retention", priority: 5, description: "Remember what you learn" },
  "problem-solving": { label: "Problem-solving skills", priority: 6, description: "Tackle complex challenges" },
  "research-skills": { label: "Research methodology", priority: 7, description: "Develop research capabilities" },
  "career-prep": { label: "Career preparation", priority: 8, description: "Prepare for your profession" }
};
...
export default function OnboardingPage() {
  ...
  const availableSubjects = useMemo((): string[] => {
    ...
    if (studentData.subjects.length > 0) {
      ...
      Object.entries(SUBJECT_CONFIGS).forEach(([subject, subjectConfig]) => {
        if (userCategories.has(subjectConfig.category) && !baseSubjects.includes(subject)) {
  ...
  const availableGoals = useMemo((): Array<{ value: StudyGoal; label: string; description: string; priority: number }> => {
    if (!studentData.educationLevel) return [];
  ...
  const contextualizedExamLabel = useMemo((): string => {
    ...
    const timeframeText = examConfig.timeframe === "short" ? "soon" : examConfig.timeframe === "medium" ? "this semester" : "this year";
    return `Preparing for ${studentData.examType} ${timeframeText}?`;
  }, [studentData.examType]);

  const smartSubjectGroups = useMemo((): Array<{ category: string; subjects: string[] }> => {
  ...
  const handleStudentDataChange = (field: keyof StudentData, value: string | string[]): void => {
            ...
            <Card className="p-8">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <Select onValueChange={(value: EducationLevel) => handleStudentDataChange("educationLevel", value)}>
                    <SelectTrigger>
                ...
                {studentData.educationLevel && (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-500 mb-3">
                      Select subjects relevant to your {studentData.educationLevel.toLowerCase()} studies
                    </p>
                    {smartSubjectGroups.map(group => (
                      <div key={group.category} className="space-y-2">
                        <h4 className="text-sm font-medium text-gray-700">{group.category}</h4>
                ...
                {availableExams.length > 0 && (
                  <div className="space-y-2">
                    ...
                    <Select onValueChange={(value: string) => handleStudentDataChange("examType", value)}>
                      <SelectTrigger>
                            ...
                            <div className="flex flex-col items-start">
                              ...
                              {EXAM_CONFIGS[exam] && (
                                <span className="text-xs text-gray-500">
                                  {EXAM_CONFIGS[exam].timeframe} term • {EXAM_CONFIGS[exam].category}
                                </span>
                              )}
                            </div>
                          </SelectItem>
                ...
                {availableGoals.length > 0 && (
                  <div className="space-y-2">
                    ...
                    <Select onValueChange={(value: StudyGoal) => handleStudentDataChange("studyGoals", value)}>
                      <SelectTrigger>
